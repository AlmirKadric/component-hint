...
function ComponentHint(options) {
	...
	// List of all local and external component scripts (defined and required)
	this.mainScripts = {};
	this.definedScripts = {};
	this.usedScripts = {};
	...
	async.series([
		function (callback) {
			...
			fs.readFile(jsonFilename, function (error, data) {
				...
				// Make sure missing main script is injected into scripts array
				var mainScript = componentJson.main = componentJson.main || 'index.js';
				var scripts = componentJson.scripts || [];
				if (scripts.indexOf(mainScript) < 0) {
		...
		function (callback) {
			// Try and load the script files
			var scriptFiles = componentJson.scripts || ['index.js'];
			async.eachLimit(scriptFiles, 20, function (scriptFile, fileCB) {
				var scriptFilename = path.join(componentPath, scriptFile);
				fs.readFile(scriptFilename, 'utf8', function (error, data) {
					if (error) {
						self.emit(eventChannel, componentPath, 'Could not load script file: ' + scriptFilename);
						return fileCB();
					}

					scriptsData[scriptFile] = data;
					return fileCB();
				});
			}, callback);
		}
